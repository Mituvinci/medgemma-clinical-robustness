#!/bin/bash
#
# Run JAADCR Evaluations - MedGemma-27B-IT Vertex AI Endpoint
#
...
#
# BEFORE RUNNING:
#   1. Deploy MedGemma-27B-IT on Vertex AI Model Garden
#   2. Update src/agents/registry.py:
#      - "medgemma-27b-it-vertex" -> status: "active"
...
#
# Usage (no GPU needed):
#   bash bin/run_evaluation_jdcr_27b_vertex.sh
#
...
echo "========================================================================"
echo "  MedGemma Evaluation Pipeline - MedGemma-27B-IT Vertex AI (JAADCR)"
...
# Verify 27B endpoint is configured (not placeholder)
ENDPOINT_27B=$(grep -A 10 '"medgemma-27b-it-vertex"' src/agents/registry.py | grep 'endpoint_id' | head -1)
if echo "$ENDPOINT_27B" | grep -q "UPDATE_AFTER_DEPLOY"; then
    echo ""
    echo "ERROR: endpoint_id for medgemma-27b-it-vertex is not configured!"
    echo "  1. Deploy MedGemma-27B-IT on Vertex AI Model Garden"
    echo "  2. Update src/agents/registry.py with the new endpoint_id and region"
    echo "  3. Set status to 'active'"
    exit 1
fi

STATUS_27B=$(grep -A 3 '"medgemma-27b-it-vertex"' src/agents/registry.py | grep 'status' | head -1)
if echo "$STATUS_27B" | grep -q "stub"; then
    echo ""
    echo "ERROR: medgemma-27b-it-vertex status is 'stub'!"
    echo "  Update status to 'active' in src/agents/registry.py"
    exit 1
fi

echo "27B-IT Vertex endpoint: configured"
echo ""
...
python scripts/evaluate_jdcr_cases.py \
    ...
    --resume \
    --max-cases 25 \
    --output logs/jaadcr_evaluated/evaluation_medgemma-27b-it-vertex_without_options
echo "Finished: $(date)"
echo ""
...
python scripts/evaluate_jdcr_cases.py \
    ...
    --resume \
    --max-cases 25 \
    --output logs/jaadcr_evaluated/evaluation_medgemma-27b-it-vertex_with_options
echo "Finished: $(date)"
echo ""

echo "========================================================================"
echo "  MedGemma-27B-IT JAADCR EVALUATIONS COMPLETE"
echo "========================================================================"
echo "Finished: $(date)"
echo ""
echo "Results saved to:"
echo "  logs/jaadcr_evaluated/evaluation_medgemma-27b-it-vertex_without_options/"
echo "  logs/jaadcr_evaluated/evaluation_medgemma-27b-it-vertex_with_options/"
echo ""
