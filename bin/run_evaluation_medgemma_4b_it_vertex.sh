#!/bin/bash
#
# Run NEJM Evaluations - MedGemma-4B-IT Vertex AI Endpoint
#
# 250 total: 25 cases x 5 variants x 2 data paths x 1 model
#
# Model:
#   MedGemma-4B-IT (Vertex AI endpoint - no local GPU needed)
#
...
#   bash bin/run_evaluation_4B_vertex.sh
#
# Usage (batch job - CPU-only partition is fine):
#   sbatch bin/run_evaluation_4B_vertex.sh
#
#SBATCH --partition=shared
#SBATCH --mem=16G
#SBATCH --time=24:00:00
#SBATCH --output=logs/evaluation_4B_vertex_job_%j.log
#SBATCH --job-name=medgemma_4B_eval_vertex
...
echo "========================================================================"
echo "  MedGemma Evaluation Pipeline - MedGemma-4B-IT Vertex AI"
...
# Verify 4B endpoint is configured
ENDPOINT_4B=$(grep -A 10 '"medgemma-4b-it-vertex"' src/agents/registry.py | grep 'endpoint_id' | grep -o '"mg-endpoint[^"]*"' | tr -d '"')
echo "Vertex 4B endpoint: ${ENDPOINT_4B:-NOT SET}"
if [ -z "$ENDPOINT_4B" ]; then
    echo "ERROR: endpoint_id for medgemma-4B-it-vertex not configured in src/agents/registry.py"
    echo "Deploy MedGemma-4B-IT first and update the endpoint_id."
    exit 1
fi
echo ""

echo "========================================================================"
echo "  RUN 1/4: MedGemma-4B-IT (Vertex AI) + without options"
echo "========================================================================"
echo "Started: $(date)"
python scripts/evaluate_nejim_cases.py \
    ...
    --resume \
    --output logs/nejm_evaluated/evaluation_medgemma-4B-it-vertex_without_options
echo "Finished: $(date)"
echo ""
...
echo "========================================================================"
echo "  RUN 2/4: MedGemma-4B-IT (Vertex AI) + with options (MCQ)"
echo "========================================================================"
echo "Started: $(date)"
python scripts/evaluate_nejim_cases.py \
    ...
    --agent-model medgemma-4b-it-vertex \
    --resume \
    --output logs/nejm_evaluated/evaluation_medgemma-4B-it-vertex_with_options
echo "Finished: $(date)"
echo ""
...
echo "========================================================================"
echo "  RUN 3/4: MedGemma-4B-IT (Vertex AI) + without options"
echo "========================================================================"
echo "Started: $(date)"
python scripts/evaluate_jdcr_cases.py \
    ...
    --resume \
    --max-cases 25 \
    --output logs/jaadcr_evaluated/evaluation_medgemma-4B-it-vertex_without_options
echo "Finished: $(date)"
echo ""
echo "Waiting 60 seconds between runs to avoid Gemini API rate limits..."
sleep 60
...
echo "========================================================================"
echo "  RUN 4/4: MedGemma-4B-IT (Vertex AI) + with options (MCQ)"
echo "========================================================================"
echo "Started: $(date)"
python scripts/evaluate_jdcr_cases.py \
    ...
    --resume \
    --max-cases 25 \
    --output logs/jaadcr_evaluated/evaluation_medgemma-4B-it-vertex_with_options
echo "Finished: $(date)"
echo ""


echo "========================================================================"
echo "  MedGemma-4B-IT VERTEX AI EVALUATIONS COMPLETE"
echo "========================================================================"
echo "Finished: $(date)"
echo ""
echo "Results saved to:"
echo "  logs/nejm_evaluated/evaluation_medgemma-4B-it-vertex_without_options/"
echo "  logs/nejm_evaluated/evaluation_medgemma-4B-it-vertex_with_options/"
echo "  logs/jaadcr_evaluated/evaluation_medgemma-4B-it-vertex_without_options/"
echo "  logs/jaadcr_evaluated/evaluation_medgemma-4B-it-vertex_with_options/"
echo ""
echo "Summary:"
echo "  - 250 total evaluations (125 + 125)"
echo "  - Model: MedGemma-4B-IT (Vertex AI endpoint)"
